...
Map::Map(int mapColumns, int mapRows, int* levelData,
    ...
    mTextureRows {textureRows}, mTileSpacing {tileSpacing}, mOrigin {origin} {
    addNonSolidTile(-1);
    build();
}
...
void Map::build() {
    ...
    int tilePixelSize =
        (mTextureAtlas.width - (mTextureColumns - 1) * mTileSpacing)
        / mTextureColumns;

    for (int row = 0; row < mTextureRows; row++) {
        for (int col = 0; col < mTextureColumns; col++) {
            Rectangle textureArea = {
                (float)col * (tilePixelSize + mTileSpacing),
                (float)row * (tilePixelSize + mTileSpacing),
                (float)tilePixelSize, (float)tilePixelSize};
...
void Map::render() {
    ...
    for (int row = 0; row < mMapRows; row++) {
        ...
        for (int col = 0; col < mMapColumns; col++) {
            ...
            // If the tile index is -1, we do not draw anything
            if (tile == -1) continue;
...
bool Map::isSolidTileAt(Vector2 position, float* xOverlap, float* yOverlap) {
    ...
    *yOverlap = 0.0f;
    ...
    if (mNonSolidTiles.count(tile)) return false;
...
void Map::panCamera(Camera2D* camera, const Vector2* targetPosition,
    ...
    camera->target.x = Clamp(camera->target.x, mLeftBoundary + halfScreenWidth,
                             mRightBoundary - halfScreenWidth);

    camera->target.y = Clamp(camera->target.y, mTopBoundary + halfScreenHeight,
                             mBottomBoundary - halfScreenHeight);
}