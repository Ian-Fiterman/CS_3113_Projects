...
void LevelA::initialise() {
    ...
    */
    ...
    mGameState.lilGuy =
        new Player(mGameState.map->getTileCenter(10, 1), {64, 64},
    ...
    */
    ...
    Wanderer* wanderer1 =
        new Wanderer(mGameState.map->getTileCenter(5, 13), {64, 64},
    ...
    Wanderer* wanderer2 =
        new Wanderer(mGameState.map->getTileCenter(9, 18), {64, 64},
                     "assets/game/spritesheet-enemies-default.png", ATLAS,
                     {8, 8}, wandererAtlas);
    wanderer2->setDirection(LEFT);
    wanderer2->setAcceleration({0.0f, ACCELERATION_OF_GRAVITY});
    mGameState.enemies.push_back(wanderer2);

    Follower* follower =
        new Follower(mGameState.map->getTileCenter(12, 8), {64, 64},
                     "assets/game/spritesheet-enemies-default.png", ATLAS,
                     {8, 8}, followerAtlas);
    ...
    */
    ...
    mGameState.flagpole = new Flagpole(
        mGameState.map->getTileCenter(7, 22), {64, 64},
...
void LevelA::update(float deltaTime) {
    if (mGameState.winTimer > 0.0f) {
        ...
        if (mGameState.winTimer <= 0.0f) {
            mGameState.nextSceneID = SCENE_B;
            return;
        }
    }
